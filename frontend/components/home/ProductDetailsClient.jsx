// components/home/ProductDetailsClient.jsx
"use client";
  ...
  const tabBtn = (key, label) => (
    <button
      ...
      className={`px-4 py-2 rounded-lg text-sm font-medium ${
        tab === key ? "bg-gray-200 text-gray-900" : "text-gray-600 hover:bg-gray-100"
      }`}
    >
      {label}
    </button>
    ...
    <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl shadow p-4 sm:p-6">
          ...
          {category?.name && <p className="text-sm text-gray-600 mb-1">Category: {category.name}</p>}
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-2 flex-wrap">
                {product.colors.map((c) => (
                  <button
                    key={c.name}
                    onClick={() => { setActiveColor(c.name); setActiveIdx(0); }}
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              {discountPct && <span className="text-red-500 font-semibold">-{discountPct}%</span>}
            </div>
            <button
              onClick={() => toggleWishlist(product._id)}
              className={`p-3 rounded-full shadow ${isInWishlist ? "bg-red-500 text-white" : "bg-gray-200 text-gray-600"}`}
            >
              <FaHeart />
            </button>
          </div>
      ...
      {quantity > 0 && (
        <div className="flex-1 md:flex-[2] flex flex-col gap-1">
          ...
          <p className="text-sm font-medium">
            Total: <span className="text-blue-600 font-semibold">৳{totalPrice}</span>
          </p>
        </div>
      )}
                ...
                product.reviews.map((r, i) => (
                  <div key={i} className="border-b py-2">
                    <p className="font-semibold">
                      {r.user} <span className="text-yellow-500">{"★".repeat(r.rating)}</span> <span className="text-gray-500">{r.rating}/5</span>
                    </p>
                    <p>{r.comment}</p>
                  </div>
                ))
      ...
      {related?.length > 0 && (
        <section className="mt-10">
          <div className="flex items-center justify-between mb-4">
            ...
            {category && (
              <Link href={`/categories/${category._id}`} className="text-blue-600 hover:underline text-sm">
